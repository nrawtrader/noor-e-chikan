...
const FilterSortBar = ({ filtersOpen, setFiltersOpen, itemCount }: FilterSortBarProps) => {
  ...
  const categories = ["Kurtas", "Dupattas", "Sarees", "Suits"];
  const priceRanges = ["Under ₹3,000", "₹3,000 - ₹6,000", "₹6,000 - ₹10,000", "Over ₹10,000"];
  const fabrics = ["Cotton", "Georgette", "Chiffon", "Silk", "Linen"];
          ...
          <div className="flex items-center gap-4">
            <Sheet open={filtersOpen} onOpenChange={setFiltersOpen}>
              <SheetTrigger asChild>
                <Button variant="ghost" size="sm" className="font-light hover:bg-transparent">Filters</Button>
              </SheetTrigger>
              <SheetContent side="right" className="w-80 bg-background border-none shadow-none">
                ...
                <div className="space-y-8">
                    ...
                    <div className="space-y-3">
                      {categories.map((category) => (
                        <div key={category} className="flex items-center space-x-3">
                          ...
                          <Label htmlFor={category} className="text-sm font-light text-foreground cursor-pointer">{category}</Label>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="space-y-3">
                      {fabrics.map((fabric) => (
                        <div key={fabric} className="flex items-center space-x-3">
                          ...
                          <Label htmlFor={fabric} className="text-sm font-light text-foreground cursor-pointer">{fabric}</Label>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-2 pt-4">
                    <Button variant="ghost" size="sm" className="w-full border-none hover:bg-transparent hover:underline font-normal text-left justify-start">Apply Filters</Button>
                    <Button variant="ghost" size="sm" className="w-full border-none hover:bg-transparent hover:underline font-light text-left justify-start">Clear All</Button>
                  </div>
                </div>
              </SheetContent>
            </Sheet>
