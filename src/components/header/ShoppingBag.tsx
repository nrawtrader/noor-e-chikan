          ...
          {onViewFavorites && (
            <div className="md:hidden mb-6 pb-6 border-b border-border">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3 px-4 border border-border text-nav-foreground hover:text-nav-hover hover:border-nav-hover transition-colors duration-200"
              >
            ...
            <>
                  ...
                  <div key={`${item.id}-${item.size}`} className="flex gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-start mb-1">
                        <div>
                          <p className="text-xs font-display text-muted-foreground uppercase tracking-wide">{item.category}</p>
                        ...
                        <p className="text-sm font-light text-foreground shrink-0 ml-2">{item.price}</p>
                      </div>
                      <div className="flex items-center gap-2 mt-3">
                        <div className="flex items-center border border-border">
                          <button onClick={() => updateQuantity(item.id, item.size, item.quantity - 1)} className="p-2 hover:bg-muted/50 transition-colors" aria-label="Decrease">
                            <Minus size={12} />
                          </button>
                          <span className="px-3 py-2 text-sm font-light min-w-[36px] text-center">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.id, item.size, item.quantity + 1)} className="p-2 hover:bg-muted/50 transition-colors" aria-label="Increase">
                            <Plus size={12} />
                          </button>
                        </div>
                      </div>
              ...
              <div className="border-t border-border pt-6 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-medium text-foreground">₹{totalPrice.toLocaleString("en-IN")}</span>
                </div>
